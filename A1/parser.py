...
class PartialParse(object):
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        """
        # *** BEGIN YOUR CODE ***
        if transition_id != 0 and transition_id != 1 and transition_id != 2:
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        # check left_arc
            ...
            if graph.nodes[stack_second_last_word]['head'] == stack_last_word:
                transition_id = self.left_arc_id
                for key, value in graph.nodes[stack_last_word]['deps'].items():
                    if stack_second_last_word in value:
                        deprel = key

        # check right_arc
            if graph.nodes[stack_last_word]['head'] == stack_second_last_word:
                # two case to do right_arc
                # first case: there is no dependants of stack_last_word
                num_right_deps_in_graph = len(list(get_right_deps(graph.nodes[self.stack[-1]])))
                num_left_deps_in_graph = len(list(get_left_deps(graph.nodes[self.stack[-1]])))
                if num_right_deps_in_graph == 0:
                    transition_id = self.right_arc_id
                # second case: the dependants of stack_last_word are all in the arcs
                dependants = [i[1] for i in self.arcs if i[0] == stack_last_word]
                if len(dependants) == (num_right_deps_in_graph + num_left_deps_in_graph):
                    transition_id = self.right_arc_id
                for key, value in graph.nodes[stack_second_last_word]['deps'].items():
                    if stack_last_word in value:
                        deprel = key
