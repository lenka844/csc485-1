...
class ParserModel(nn.Module):
    ...
    def create_embeddings(self, word_embeddings):
        ...
        """
        # *** BEGIN YOUR CODE ***
        self.word_embeddings = torch.tensor(word_embeddings)
    ...
    def create_weights_biases(self):
        ...
        self.W_o.requires_grad_(True)
        self.b_o.requires_grad_(True)
        # *** END YOUR CODE ***
    ...
    def embedding_lookup(self, id_batch, n_ids, embedding_matrix):
        ...
        """
        # *** BEGIN YOUR CODE ***
        one_hot_vectors = one_hot_float(id_batch, n_ids)
        B = id_batch.size()[0]
        N = id_batch.size()[1]
        embedded_batch = torch.mm(torch.reshape(one_hot_vectors, (-1, n_ids)), embedding_matrix)
        embedded_batch = torch.reshape(embedded_batch, (B, N, -1))
    ...
    def get_concat_embeddings(self, word_id_batch, tag_id_batch,
        ...
        """
        # *** BEGIN YOUR CODE ***
        word_embedded_batch = self.embedding_lookup(word_id_batch, self.config.n_word_ids, self.word_embeddings)
        tag_embedding_batch = self.embedding_lookup(tag_id_batch, self.config.n_tag_ids, self.tag_embeddings)
        deprel_embedding_batch = self.embedding_lookup(deprel_id_batch, self.config.n_deprel_ids, self.deprel_embeddings)
        x = torch.cat((word_embedded_batch, tag_embedding_batch, deprel_embedding_batch), 1)
        # *** END YOUR CODE ***
        return x
    ...
    def add_optimizer(self):
        ...
        """
        # *** BEGIN YOUR CODE ***
        params = [self.word_embeddings, self.tag_embeddings, self.deprel_embeddings, self.W_h, self.b_h, self.W_o, self.b_o]
